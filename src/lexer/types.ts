import type { CollectionCreateSchema, CollectionFieldSchema } from "../collection";
import type { OmitDefaultSortingField } from "../lib/utils";
import type { Char, Digit, EOF, Number, TokenMap } from "./tokens";
...
type FieldTypeMap<
  ...
  >,
> = {
  [K in keyof T["fields"]]: T["fields"][K]["type"];
};
...
> =
  TokenArray extends [infer Head, ...infer Tail] ?
    Head extends Types[0] ?
      CheckBalancedTokens<Token, Extract<Tail, Token[]>, Types, [Types[0], ...Stack]>
    : Head extends Types[1] ?
      Stack extends [Types[0], ...infer Rest] ?
        CheckBalancedTokens<Token, Extract<Tail, Token[]>, Types, Extract<Rest, Token[]>>
      : false
